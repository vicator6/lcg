...
if ( !class_exists( 'LPR_Order_Post_Type' ) ) {
	...
	final class LPR_Order_Post_Type {
		...
		function __construct() {
			...
			/*Add Coulumn*/
			add_filter( 'manage_edit-lpr_order_columns', array( $this, 'learn_press_order_columns' ) );
...
*/
		 ...
		 *
		 */
		function learn_press_order_columns( $existing ) {
		...
		function render_learn_press_order_columns( $column ) {
			...
			switch ( $column ) {
				case 'order_student':
					$user = get_user_by( 'id', get_post_meta( $post->ID, '_learn_press_customer_id', true ) );
		...
		function register_post_type() {
			...
			register_post_type( LPR_ORDER_CPT,
				array(
					...
					'hierarchical'    => true,
					'rewrite'         => array( 'slug' => _x( 'lpr_order', 'Permalink Slug', 'learn_press' ), 'hierarchical' => true, 'with_front' => true )
				)
			);
				...
				<div class="order-user-data clearfix">
					...
					<div class="order-user-meta">
						<h2 class="user-display-name">
							<?php echo ( $display_name = $user->display_name ) && !empty( $display_name ) ? $display_name : __( 'Unknown', 'learn_press' ); ?>
						</h2>

						<div class="user-email">
							<?php echo ( $user_email = $user->user_email ) && !empty( $user_email ) ? $user_email : __( 'Unknown', 'learn_press' ); ?>
						</div>
		...
		function register_post_statues() {
			...
			register_post_status( 'lp-cancelled', array(
				'label'                     => _x( 'Cancelled Payment', 'Order status', 'learn_press' ),
				'public'                    => false,
				'exclude_from_search'       => false,
				'show_in_admin_all_list'    => true,
				'show_in_admin_status_list' => true,
				'label_count'               => _n_noop( 'Cancelled Payment <span class="count">(%s)</span>', 'Cancelled Payment <span class="count">(%s)</span>', 'learn_press' )
			) );
