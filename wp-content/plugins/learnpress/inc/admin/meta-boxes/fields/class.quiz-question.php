...
if ( !class_exists( 'RWMB_Quiz_Question_Field' ) ) {
	class RWMB_Quiz_Question_Field extends RWMB_Field {
		...
		static function quiz_question_add() {
			...
			$question_id = isset( $_REQUEST['question_id'] ) ? $_REQUEST['question_id'] : null;
			//$question    = LPR_Question_Type::instance( $question_id ? $question_id : $type );//->get_question( $question_id ? $question_id : $type );
			$question    = LPR_Question_Factory::instance()->get_question( $question_id, array( 'text' => $text, 'type' => $type ) );//->get_question( $question_id ? $question_id : $type );
			$json        = array(
				'success' => false
			);
			if ( $question ) {
				if ( isset( $_POST['quiz_id'] ) && ( $quiz_id = $_POST['quiz_id'] ) ) {
					...
					$quiz_questions[$question->id] = array( 'toggle' => 0 );
				...
				$json['success']  = true;
				$json['question'] = get_post( $question->id );
			} else {
		...
		static function html( $meta, $field ) {
			...
			$questions = LPR_Question_Factory::get_types();
			...
			ob_start();
			?>
			<script type="text/javascript">var learn_press_quiz_id = <?php echo intval( $post_id ); ?></script>
				...
				<div id="lpr-quiz-questions">
					<?php if ( $qids ): $index = 0; ?>
						<?php foreach ( $qids as $question_id ): ?>
							<?php
							if ( $question_id && $question = LPR_Question_Factory::instance()->get_question( $question_id ) /* LPR_Question_Factory::instance()->get_question( $question_id )*/ ) {
					...
					<ul class="dropdown-menu" role="menu">
						<?php if ( $questions ): ?>
							<?php foreach ( $questions as $type ): ?>
								<li>
									<a href="" rel="<?php echo $type; ?>"><?php echo learn_press_question_slug_to_title($type); ?></a>
								</li>
							<?php endforeach; ?>
						<?php endif; ?>
					</ul>
