...
class LPR_Gateway_Paypal extends LPR_Gateway_Abstract {
	...
	function process_order_paypal_standard_secure() {
		if ( !empty( $_REQUEST['learn-press-transaction-method'] ) && ( 'paypal-standard-secure' == $_REQUEST['learn-press-transaction-method'] ) ) {
			...
			if ( !empty( $_REQUEST['paypal-nonce'] ) && wp_verify_nonce( $_REQUEST['paypal-nonce'], 'learn-press-paypal-nonce' ) ) {
				...
				if ( $custom )
					$transient_transaction_id = $custom['temp_id'];
				else
			...
			if( $custom ){
				$redirect = get_the_permalink( $custom['course_id'] );
			}
	...
	function process_order_paypal_standard() {
		if ( !empty( $_REQUEST['learn-press-transaction-method'] ) && ( 'paypal-standard' == $_REQUEST['learn-press-transaction-method'] ) ) {
			...
			if( $custom ) $redirect = get_the_permalink( $custom['course_id'] );
			wp_redirect( $redirect );
			die();
		}
	...
	function get_request_url( $order ) {
		...
		if ( !empty( $paypal_email )
			...
			$custom      = array( 'temp_id' => $temp_id, 'course_id' => learn_press_get_cart_course_id() );
			...
			$L_BUTTONVARS[] = 'custom=' . json_encode( $custom );//$temp_id;
	...
	function _parse_custom( $raw_custom = '' ) {
		if( empty( $raw_custom ) ) {
			if ( !empty( $_REQUEST['custom'] ) ) {
				$raw_custom = $_REQUEST['custom'];
			} elseif ( !empty( $_REQUEST['cm'] ) ) {
				$raw_custom = $_REQUEST['cm'];
			}
		}
		$raw_custom = stripslashes( $raw_custom );
		if ( ( $custom = json_decode( $raw_custom ) ) && ( is_object( $custom ) || is_a($custom) ) ) {
		...
		return array( 'course_id' => $course_id, 'temp_id' => $temp_id );
	}
