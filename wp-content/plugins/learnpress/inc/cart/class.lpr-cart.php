...
class LPR_Cart {
	...
	function __construct() {
		...
		if ( empty( $_SESSION['learn_press_cart'] ) || !is_array( $_SESSION['learn_press_cart'] ) ) {
...
function learn_press_get_cart_course_id() {
	$products = learn_press_get_cart( 'products' );
	$return   = '';
	if ( $products ) {
		foreach ( $products as $prop ) {
	...
	return apply_filters( 'learn_press_cart_course_id', $return );
}
