...
function learn_press_get_current_profile_link() {
	...
	$link = home_url( "/profile/" . urlencode( $current_user->user_login ) );
...
function learn_press_get_profile_link( $link, $user_id, $course_id ) {
	...
	$link       = home_url( "/profile/" . urlencode( $user_login ) );
...
function learn_press_query_taxonomy( $q ) {
	...
	if ( is_search() ) {
		...
		add_filter( 'wp', 'learn_press_remove_query_tax' );
	}
}
...
function learn_press_remove_query_tax() {
 ...
 * Only search learn_press_course if see the param ref=course in request
 *
 * @param $q
 */
...
add_filter( 'parse_query', 'learn_press_filter_search' );
...
function learn_press_get_order_statuses() {
	$order_statuses = array(
		'lp-pending'    => _x( 'Pending', 'Order status', 'learn_press' ),
		'lp-processing' => _x( 'Processing', 'Order status', 'learn_press' ),
		'lp-completed'  => _x( 'Completed', 'Order status', 'learn_press' ),
		'lp-on-hold'    => _x( 'On Hold', 'Order status', 'learn_press' ),
		'lp-refunded'   => _x( 'Refunded', 'Order status', 'learn_press' ),
		'lp-failed'     => _x( 'Failed', 'Order status', 'learn_press' ),
		'lp-cancelled'  => _x( 'Cancelled', 'Order status', 'learn_press' )
	);
	return apply_filters( 'learn_press_order_statuses', $order_statuses );
}

function learn_press_question_slug_to_title( $slug ) {
	$title = __( 'None', 'learn_press' );
	if ( $slug ) {
		$title = ucwords( preg_replace( '!_!', ' ', $slug ) );
	}
	return apply_filters( 'learn_press_question_slug_to_title', $title, $slug );
}

function learn_press_debug( $a, $b = true ) {
	echo '<pre>';
	print_r( $a );
	echo '</pre>';
	if ( $b ) {
...
function thim_remove_course_slug( $post_link, $post, $leavename ) {
	if ( 'lpr_course' != $post->post_type || 'publish' != $post->post_status ) {
		return $post_link;
	}
	$post_link = str_replace( '/' . 'courses' . '/', '/', $post_link );
	return $post_link;
}

//add_filter( 'post_type_link', 'thim_remove_course_slug', 10, 3 );

function thim_add_rewrite_rules() {
	$url  = add_query_arg( '', '' );
	$path = explode( '/', $url );
	$path = array_reverse( $path );

	add_rewrite_rule( '^(course-2)/?', 'index.php?lpr_course=$matches[1]', 'top' );
}

//add_action( 'init', 'thim_add_rewrite_rules' );