...
class LPR_Question_Factory {
	...
	public function get_question( $the_question = false, $args = array() ) {
		$the_question = $this->get_question_object( $the_question, $args );
		...
		if ( !class_exists( $classname ) ) {
			$classname = 'LPR_Question_Type_None';
		}
		if ( is_array( $args ) ) {
			ksort( $args );
	...
	public function get_classname_from_question_type( $question_type ) {
		return $question_type ? 'LPR_Question_Type_' . implode( '_', array_map( 'ucfirst', explode( '-', $question_type ) ) ) : false;
	}

	/**
	 * Get the type of question stored in database by it ID
	 * Also, accepts a value stored in learn_press_question meta format
	 *
	 * @param $the_question
	 * @param $args
	 *
	...
	public function get_question_type( $the_question, $args = array() ) {
		if ( !empty( $args['type'] ) ) {
			...
			if ( is_numeric( $the_question ) ) {
				$options = (array) get_post_meta( $the_question, '_lpr_question', true );
			} else {
	...
	public function get_question_class( $the_question, $args = array() ) {
		...
		return apply_filters( 'form_press_field_class', $classname, $question_type, $question_id );
	}
	...
	public function get_question_object( $the_question, $args = array() ) {
		...
		if ( !$the_question && !empty( $args['text'] ) && !empty( $args['type'] ) ) {
			...
			$the_question = get_post( $id );
			update_post_meta( $id, '_lpr_question', array( 'type' => $args['type'] ) );
			update_post_meta( $id, '_lpr_question_mark', 1 );
		}
	...
	public static function enqueue_assets() {
		...
		if ( get_post_type() === 'lpr_quiz' || get_post_type() === 'lpr_question' ) {
	...
	static function convert_question( $id, $from, $to ) {
		$question      = LPR_Question_Factory::instance()->get_question( $id );
		$question_meta = (array) get_post_meta( $id, '_lpr_question', true );
