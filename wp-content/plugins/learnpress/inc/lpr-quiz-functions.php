 ...
 */
function learn_press_save_question_answer_bak( $quiz_id, $question_id, $question_answer ) {
 ...
 * @return bool
 */
function learn_press_check_is_question_answered( $quiz_id, $question_id ) {
	$user_id        = get_current_user_id();
	$student_answer = get_user_meta( $user_id, '_lpr_quiz_question_answer', true );
 ...
 * @return mixed|void
 */
function learn_press_get_question_answer( $quiz_id, $question_id ) {
	$user_id        = get_current_user_id();
	$student_answer = get_user_meta( $user_id, '_lpr_quiz_question_answer', true );
 ...
 * @param $quiz_id
 */
function learn_press_reset_quiz_answer( $quiz_id ) {
 ...
 * @param $result
 */
function learn_press_save_quiz_result( $quiz_id, $result ) {
	$user_id     = get_current_user_id();
 ...
 * @return mixed|void
 */
function learn_press_get_quiz_result_bak( $quiz_id ) {
	$user_id     = get_current_user_id();
	$quiz_result = get_user_meta( $user_id, '_lpr_quiz_completed', true );
	if ( $quiz_result ) {
...
function learn_press_is_final_quiz( $quiz_id ) {
	...
	if ( learn_press_get_final_quiz( $course_id ) == $quiz_id ) {
		return $course_id;
	}
	return false;
}
...
function learn_press_get_user_question_url( $quiz_id, $current_question_id = 0, $user_id = 0 ) {
	...
	if ( '' != get_option( 'permalink_structure' ) ) {
		$permalink = trailingslashit( $permalink ) . get_post_field( 'post_name', $current_question_id );
	} else {
...
function learn_press_get_user_prev_question_url( $quiz_id, $current_question_id = 0, $user_id = 0 ) {
	...
	if ( $prev_id ) {
		...
		if ( '' != get_option( 'permalink_structure' ) ) {
			$permalink = trailingslashit( $permalink ) . get_post_field( 'post_name', $prev_id );
		} else {
			$permalink .= ( strpos( $permalink, '?' ) === false ? "?" : "&" ) . "question={$question_name}";
		}
	} else {
...
function learn_press_get_user_next_question_url( $quiz_id, $current_question_id = 0, $user_id = 0 ) {
	...
	if ( $next_id ) {
		...
		if ( '' != get_option( 'permalink_structure' ) ) {
			$permalink = trailingslashit( $permalink ) . get_post_field( 'post_name', $next_id );
		} else {
			$permalink .= ( strpos( $permalink, '?' ) === false ? "?" : "&" ) . "question={$question_name}";
		}
	} else {
...
function learn_press_get_user_quiz_questions( $quiz_id = null, $user_id = null ) {
	...
	if ( $questions && !empty( $questions[ $quiz_id ] ) ) {
		...
		if( $questions ) foreach( $questions as $id ){
			$user_quiz_questions[ $id ] = $id;
		}
	} else {
		$user_quiz_questions = learn_press_get_quiz_questions( $quiz_id );
	}
	return apply_filters( 'learn_press_get_user_quiz_questions', $user_quiz_questions, $quiz_id, $user_id );
}