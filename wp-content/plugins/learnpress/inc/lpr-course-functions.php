 ...
 * @return int
 */
function learn_press_get_number_lesson( $course_id ) {
	$number_lesson     = 0;
 ...
 * @return float|int
 */
function learn_press_course_evaluation( $course_id ) {
 ...
 * @return float|int
 */
function learn_press_course_auto_evaluation( $course_id ) {
	$result            = - 1;
	...
	if ( ( $current - $start_date ) / ( 7 * 24 * 3600 ) > $course_duration ) {
		$result = learn_press_course_evaluation( $course_id );
	}
	return $result;
}
...
function learn_press_get_quiz_result( $user_id = null, $quiz_id = null ) {
	...
	$quiz_id   = $quiz_id ? $quiz_id : ( $quiz ? $quiz->id : 0 );
	$questions = learn_press_get_user_quiz_questions( $quiz_id );// learn_press_get_quiz_questions( $quiz_id );
	...
	$mark_total = 0;// learn_press_get_quiz_mark( $quiz_id );
	...
	if ( $questions ) {
		foreach ( $questions as $question_id => $options ) {
			$ques_object = LPR_Question_Factory::instance()->get_question( $question_id );
			$mark_total += ( $question_mark = get_post_meta( $question_id, '_lpr_question_mark', true ) ) ? $question_mark : 1 ;
		...
		$info = array(
			...
			'mark_total'      => $mark_total ? round( $mark_total, 2 ) : 0,
			'mark_percent'    => $mark_total ? round( $mark / $mark_total, 2 ) : 0,
			'correct_percent' => $question_count ? round( $correct_questions / $question_count * 100, 2 ) : 0,
			'wrong_percent'   => $question_count ? round( $wrong_questions / $question_count * 100, 2 ) : 0,
			'empty_percent'   => $question_count ? round( $empty_questions / $question_count * 100, 2 ) : 0,
			'quiz_time'       => $quiz_time
		);
...
function learn_press_take_course( $course_id, $payment_method = '' ) {
	...
	if ( $can_take_course && $course->id ) {
		...
		learn_press_add_message( 'error', apply_filters( 'learn_press_take_course_error_message', __( 'Sorry! You can not enroll to this course', 'learn_press' ) ) );
		$json = array(
			'result'   => 'error',
...
function learn_press_count_students_enrolled( $course_id = null ) {
	...
	if ( $student ) {
		$count = $student + $course->count_users_enrolled();
	} else {
...
function learn_press_get_user_course_status( $user_id = null, $course_id = null ) {
	...
	if ( $course_id && $user_id ) {
		...
		if ( $orders ) {
			...
			if ( ( $order = get_post( $order_id ) ) && ( !in_array( $order->post_status, array('lpr-draft', 'trash') ) ) && $order->post_type == 'lpr_order' ) {
...
function learn_press_user_can_view_quiz( $quiz_id = null, $user_id = null ) {
	...
	$return    = false;
	$course_id = learn_press_get_course_by_quiz( $quiz_id );// get_post_meta( $quiz_id, '_lpr_course', true );
...
function learn_press_get_course_by_quiz( $quiz_id, $id_only = true ) {
	...
	$query = $wpdb->prepare("
		SELECT c.*
		FROM {$wpdb->posts} c
		INNER JOIN {$wpdb->postmeta} cm ON cm.post_id = c.ID AND cm.meta_key = %s AND cm.meta_value LIKE %s

	", '_lpr_course_lesson_quiz', '%' . $wpdb->esc_like(':"'.$quiz_id.'"') . '%');
...
function learn_press_user_can_finish_course( $course_id = null, $user_id = null, $passing_condition = 0 ) {
	...
	if ( get_post_meta( $course_id, '_lpr_course_final', true ) == 'yes' ) {
		$final_quiz = learn_press_get_final_quiz( $course_id );
		$passed     = learn_press_quiz_evaluation( $final_quiz, $user_id );
		return $passed && ( $passed >= $passing_condition );
	} else {
		$passed = learn_press_course_evaluation( $course_id );
		return $passed && ( $passed >= $passing_condition );
	}
	return false;
}
...
function learn_press_user_has_completed_all_parts( $course_id = null, $user_id = null ) {
	...
	if ( get_post_meta( $course_id, '_lpr_course_final', true ) == 'yes' ) {
		...
		return learn_press_course_evaluation( $course_id ) == 100;
	}
}
...
function learn_press_user_has_passed_conditional( $course_id = null, $user_id = null ) {
	...
	if ( get_post_meta( $course_id, '_lpr_course_final', true ) == 'yes' && $quiz = learn_press_get_final_quiz( $course_id ) ) {
 ...
 * @return array
 */
function learn_press_get_related_courses( $limit ) {
	if ( !$limit ) {
		$limit = 3;
	}
	$course_id = get_the_ID();
...
function learn_press_get_question_hint( $quiz_id, $question_id, $user_id ) {
	/*
	...
	*/
	...
	$quizzes[$quiz_id][$question_id] = 1;
	update_user_meta( $user_id, '_lpr_quiz_question_hint', $quizzes );
	...
	?>
	<?php if ( $ques->get( 'options.explaination' ) ) : ?>
		<h4 class="explantion-title"><?php _e( "Answer explanation", 'learn_press' ) ?></h4>
		<p class="explantion-content"><?php echo $ques->get( 'options.explaination' ) ?></p>
	<?php endif; ?>
	<?php
	return ob_get_clean();
}

function learn_press_user_has_checked_answer( $question_id, $user_id, &$quiz_id ) {
	//$user_id = learn_press_get_current_user_id();
	$checked = false;
	$quizzes = get_user_meta( $user_id, '_lpr_quiz_question_hint', true );
	if ( $quizzes ) {
		foreach ( $quizzes as $quiz_id => $questions ) {
