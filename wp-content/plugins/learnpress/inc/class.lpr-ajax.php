...
if ( !class_exists( 'LPR_AJAX' ) ) {
	...
	class LPR_AJAX {
		...
		public static function list_quiz() {
			if ( file_exists( $template = learn_press_locate_template_part( 'single', 'quiz' ) ) ) {
		...
		private static function save_question_if_needed() {
			...
			$current_question_id = !empty( $_REQUEST['question_id'] ) ? absint( $_REQUEST['question_id'] ) : 0;

			$question = $question_id ? LPR_Question_Factory::instance()->get_question( $question_id ) : false;
			if ( $question ) {
				$check_quiz = null;

				if( !learn_press_user_has_checked_answer( $question_id, $user_id, $check_quiz )  ) {
					$question_answer = isset( $_REQUEST['question_answer'] ) ? $_REQUEST['question_answer'] : null;
					$question_answer = apply_filters( 'learn_press_submit_user_answer', $question_answer, $user_id, $question_id, $quiz_id );
					$question->submit_answer( $quiz_id, $question_answer );

					if ( $current_question_id ) {
						$current_questions           = (array) get_user_meta( $user_id, '_lpr_quiz_current_question', true );
						$current_questions[$quiz_id] = $current_question_id;
						update_user_meta( $user_id, '_lpr_quiz_current_question', $current_questions );
					}
					do_action( 'learn_press_save_user_question_answer', $question_answer, $question_id, $quiz_id, $user_id, true );
				}
			} else {
				throw new Exception( sprintf( __( 'The question with ID %d does not exists', 'learn_press' ), $question_id ) );
			}
			return $question;
		}
		...
		public static function load_quiz_question() {
			...
			if ( $question = LPR_Question_Factory::instance()->get_question( $question_id ) ) {
				$quizzes = get_user_meta( $user_id, '_lpr_quiz_question_answer', true );
				$answer = null;
				ob_start();
				if ( isset( $quizzes[ $quiz_id ] ) ){
					$quiz = $quizzes[ $quiz_id ];
					if( array_key_exists( $question_id, $quiz ) ) {
						$answer = $quiz[ $question_id ];
					}
				}
				$question->render( array( 'answer' => $answer ) );
				$content = ob_get_clean();
				learn_press_send_json(
					apply_filters( 'learn_press_load_quiz_question_result',
						array(
							'result'    => 'success',
							'content'   => $content,
		...
		public static function take_free_course() {
			...
			if ( file_exists( $template = learn_press_locate_template_part( 'course', 'main' ) ) ) {
				require_once( $template );
			}
			die;
		}
		...
		public static function finish_course() {
			...
			if ( $assessment == 'yes' ) {
				...
				$final_result = learn_press_get_quiz_result( $user_id, $final_quiz );// learn_press_get_quiz_result( $final_quiz );
				if ( !empty( $final_result ) && !empty( $final_result['mark_percent'] ) && ( $final_result['mark_percent'] * 100 >= $pass ) ) {
					$finish = true;
				}
			} else {
				$progress = learn_press_course_evaluation( $course_id );
				if ( $progress >= $pass ) {
					$finish = true;
				}
			}
				...
				case 'completed':
					learn_press_send_json(
						array(
							...
							'data'    => learn_press_get_quiz_result( $user->id, $quiz_id )//$user->get_quiz_result()
						)
					);
					break;
				case 'started':
					learn_press_send_json(
						array(
							...
							'content' => $user->get_current_question( $quiz_id, 'html' )
						)
					);
					learn_press_send_json(
						array(
