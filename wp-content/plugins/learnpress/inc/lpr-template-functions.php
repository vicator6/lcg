...
if ( !function_exists( 'learn_press_print_quiz_question_content_script' ) ) {
	...
	function learn_press_print_quiz_question_content_script() {
		...
		$question = LPR_Question_Factory::instance()->get_question( $current_question_id );
		$user_id  = get_current_user_id();
		global $quiz;
		if ( $quiz && !empty( $quiz->id ) ) {
			$user = learn_press_get_current_user();
			$js   = apply_filters(
				'learn_press_single_quiz_params',
				array(
					'id'             => $quiz->id,
...
if ( !function_exists( 'learn_press_course_percentage' ) ) {
	...
	function learn_press_course_percentage() {
		if ( learn_press_user_has_completed_quiz( null ) ) {
...
if ( !function_exists( 'learn_press_frontend_single_quiz_scripts' ) ) {
	...
	function learn_press_frontend_single_quiz_scripts() {
		if ( is_single() && 'lpr_quiz' == get_post_type() ) {
			...
			wp_enqueue_script( 'single-quiz', LPR_PLUGIN_URL . '/assets/js/single-quiz.js', array( 'backbone', 'wp-util' ) );
		}
	}
}
...
if ( !function_exists( 'learn_press_course_finished_message' ) ) {
	...
	function learn_press_course_finished_message() {
		if ( learn_press_user_has_finished_course() ) {
			$quiz_id = learn_press_get_final_quiz( get_the_ID() );
...
if ( !function_exists( 'learn_press_quiz_hint' ) ) {
	...
	function learn_press_quiz_hint( $question_id ) {
		...
		if ( $ques = learn_press_get_question( $question_id ) ) {
...
if ( !function_exists( 'learn_press_course_content_next_prev_lesson' ) ) {
	...
	function learn_press_course_content_next_prev_lesson() {
		...
		$course_id = learn_press_get_course_by_lesson( get_the_ID() );
		if ( learn_press_is_enrolled_course( $course_id ) && !learn_press_user_has_enrolled_course( $course_id ) ) return;
		...
		$args = compact( 'course_id', 'prev_id', 'next_id'/*, 'prev', 'next' */ );
		learn_press_get_template( 'course/lesson-nav.php', $args );
	}
}
 ...
 * @return mixed
 */
function learn_press_page_title( $title, $sep = '', $seplocation = '' ) {
...
add_filter( 'wp_title', 'learn_press_page_title', 10, 3 );
	...
	if ( is_single() && $post_type == 'lpr_quiz' ) {
		...
		if ( learn_press_get_request( 'question' ) && $quiz_status == '' ) {
			...
			if ( learn_press_get_request( 'question' ) ) {
				...
				// ensure that current question is existing to prevent loop redirect
				if ( get_post_type( learn_press_get_current_question( $quiz_id ) ) == 'lpr_question' ) {
 ...
 * @param $id
 */
function learn_press_after_question_content( $id ) {
...
add_action( 'learn_press_after_question_content', 'learn_press_after_question_content' );