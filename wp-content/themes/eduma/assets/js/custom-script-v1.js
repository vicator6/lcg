...
(function ($) {
	...
	var thim_Menu = function () {
		...
		if ($(window).width() > 768) {
			...
			magic.css('bottom', parseInt( $('.site-header .navigation').outerHeight() ) - 64 );
	...
	jQuery('.mobile-menu-open #main-content').on('click', function () {
	...
	jQuery(function ($) {
		...
		function _before_load_lesson($view) {
			...
			var $courseContent = $('.course-content'),
				...
				content_H = $('.mfp-content').length > 0 ? $('.course-content').outerHeight() : parseInt( winH * 3 / 4),
				content_H = ( content_H > winH ) ? winH + 1 : content_H ;

			console.log( content_H, $view );
				...
				$iframe.load(function(){
					...
					$courseContent.animate({
						...
						$parent.find('.popup-title').css({
							'left' : $('.mfp-content').position().left,
							'top'  : $('.mfp-content').position().top + parseInt($('html').css('margin-top')) ,
		...
		$(document).ready(function () {
			LearnPress.Hook
				...
				.addAction('learn_press_item_content_loaded', function ($content, that) {
					var
						$courseContent = $('.course-content').append($content.css('opacity', 0)),
		...
		$(document)
			.on('clickxxxx', '.section-content .course-quiz a', function (e) {
				return;
				var elem = $(this),
					content_H = parseInt($(window).outerHeight() * 3 / 4);
